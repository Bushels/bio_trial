...
| 8     | T27  | README + design-doc shipped marker             | ✅     | README extended with file map, the new public RPCs (farmer + vendor + dashboard), a Farmer inbox (Telegram) section with the command table, the one-time webhook-setup curl, and a Farmer/Public dashboard section with the Vercel `.html`-strip gotcha. Design doc status flipped to "Shipped 2026-04-19". |

**Post-ship Codex-audit extensions (2026-04-20).** See the design doc's "Post-ship extensions" section for rationale; each row below points at its migration or commit.

| Phase | Task | Title                                          | Status | Commit / notes |
|:-----:|:----:|:-----------------------------------------------|:------:|:---------------|
| post  | T28  | Public photo consent wired end-to-end (Codex #3) | ✅   | `20260420000001_bio_trial_dashboard_expand.sql` filters `public_opt_in=true`; `farmer_register_event` gains `p_public_opt_in`; `farmer.js` shows per-event checkbox (default off). |
| post  | T29  | Git-as-source-of-truth catch-up (Codex #1)     | ✅     | `20260420000002_bio_trial_farmer_catchup.sql` — idempotently reproduces the live farmer/vendor schema + RPCs so a fresh Supabase project bootstraps to the same state as Bushel Board. |
| post  | T30  | Trial-types + plots schema (Codex #3a)         | ✅     | `20260420000003_trial_types_and_plots.sql` — `trial_fields.trial_type` + friends, `trial_plots` table, `trial_events.plot_id`. All additive, all nullable. |
| post  | T31  | Trial-types RPCs + bootstrap extension         | ✅     | `20260420000004_trial_types_rpcs.sql` — `farmer_set_trial_type` (seeds default plots, won't destroy existing), `farmer_upsert_plot` (no virtual plots by hand), `farmer_register_event` now 7-arg (`+p_public_opt_in, +p_plot_id`) — required explicit `DROP FUNCTION` before `CREATE OR REPLACE` because Postgres treats arg-count change as an overload. `farmer_bootstrap` returns plots. |
| post  | T32  | Rigor tiers on public dashboard                | ✅     | `20260420000005_dashboard_trial_tiers.sql` — `headline.rigor_tier_counts` (controlled / referenced / observational / undeclared). v1 of `aggregates_by_tier` shipped here; it's superseded by T36 below. |
| post  | T33  | Codex follow-up — relax `trial_plots` acres    | ✅     | `20260420000006_relax_trial_plots_acres.sql` — drops the `is_virtual OR acres IS NOT NULL` check that would have rejected `farmer_set_trial_type` seeding STRIP/SPLIT plots with acres=NULL. Deliberately no default-acreage backfill — fake numbers corrupt per-acre yield math worse than NULL does. Source edit in `003` so fresh DBs don't recreate the constraint. |
| post  | T34  | Farmer UI — trial-type picker + plot form      | ✅     | `farmer.js`: `TRIAL_TYPE_INFO` constant, per-field "Declare trial type" CTA and blurb, conditional inputs for historical yield / neighbor notes, plot picker on the event form (filtered to real plots; required for yield on fields that have plots). |
| post  | T35  | Trial UI — tier tiles + tier-segmented table   | ✅     | `trial.js`: four extra headline tiles when `rigor_tier_counts` is present; `renderAggregates(byTier, flatList, headline, floor)` prefers tier-segmented rows when non-empty, falls back to flat table. Pattern lets the frontend tolerate the RPC shape changing in T36. |
| post  | T36  | Next-audit P1: yield-field invariant + disable tiered aggregates | ✅ | `20260420000007_yield_field_required_and_hide_tiered.sql` — `farmer_register_event` rejects `kind='yield'` without `field_id`; `yields_count` headline tightens to field-linked yields (matches aggregate scope); `aggregates_by_tier` returns `[]` until role-aware delta math lands (v1 was blending treated+check inside tiers — a public-honesty bug). `farmer.js` mirrors the yield-field guard client-side. |
| post  | T37  | Telegram `/yield` strip/split routing          | ✅     | `bio-trial-farmer-inbox/index.ts`: `fieldsWithRealPlots` helper, picker hides fields with real plots and tells the farmer to use the web form, callback re-checks at tap time (defense in depth against stale `callback_data`). |
| post  | T38  | Post-ship docs refresh                         | ✅     | This row — README trial-types/rigor-tiers section, design-doc "Post-ship extensions" section, this progress-log extension. |
